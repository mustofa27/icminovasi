...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <h1 class="text-3xl font-bold text-gray-900 mb-8">Edit Testimonial</h1>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-8">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="project_id" id="project_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" required>
                    ...
                    @foreach($projects as $project)
                        <option value="{{ $project->id }}" {{ old('project_id', $testimonial->project_id) == $project->id ? 'selected' : '' }}>{{ $project->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="client_id" id="client_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('client_id', $testimonial->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="mb-6">
            ...
            <textarea name="testimonial" id="testimonial" rows="6" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('testimonial', $testimonial->testimonial) }}</textarea>
            ...
            <div>
                ...
                @if($testimonial->client_photo)
                    <div class="mt-3">
                        ...
                        <img src="{{ asset('storage/' . $testimonial->client_photo) }}" alt="{{ $testimonial->client_name }}" class="h-20 w-20 rounded-lg object-cover">
                    </div>
                @endif
        ...
        <div class="mb-8">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $testimonial->is_published) ? 'checked' : '' }} class="w-4 h-4">
